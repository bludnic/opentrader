...
export const GRID_LEVELS: IGridBotLevel[] = [
  ...
  {
    buy: { price: 14, status: OrderStatusEnum.Filled, quantity: ORDER_QUANTITY },
    sell: { price: 16, status: OrderStatusEnum.Idle, quantity: ORDER_QUANTITY }
  },
  {
    buy: { price: 16, status: OrderStatusEnum.Filled, quantity: ORDER_QUANTITY },
    sell: { price: 18, status: OrderStatusEnum.Idle, quantity: ORDER_QUANTITY }
  },
  {
    buy: { price: 18, status: OrderStatusEnum.Filled, quantity: ORDER_QUANTITY },
    sell: { price: 20, status: OrderStatusEnum.Idle, quantity: ORDER_QUANTITY }
  },
]

// 3 Sell Orders
export const BASE_CURRENCY_INVESTMENT = ORDER_QUANTITY + ORDER_QUANTITY + ORDER_QUANTITY
// 2 Buy Orders
export const QUOTE_CURRENCY_INVESTMENT = 12 * ORDER_QUANTITY + 10 * ORDER_QUANTITY;
