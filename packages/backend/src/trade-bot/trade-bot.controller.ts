import { Body, Controller, Get, Inject, Param, Post, Put, Scope } from "@nestjs/common";
...
import { ExchangeFactory, exchangeFactory, ExchangeFactorySymbol } from "src/core/exchanges/exchange.factory";
...
export class TradeBotController {
  ...
  @Get()
  async getBots(@FirebaseUser() user: IUser): Promise<GetTradeBotsListResponseDto> {
  ...
  @Get('/info/:id')
  async getBot(@Param('id') botId: string): Promise<GetTradeBotResponseBodyDto> {
  ...
  @Put('/start/:id')
  async startBot(@Param('id') botId: string): Promise<StartTradeBotResponseBodyDto> {
    const tradeBotService = await this.tradeBotServiceFactory.fromBotId(botId);
  ...
  @Put('/stop/:id')
  async stopBot(@Param('id') botId: string): Promise<StopTradeBotResponseBodyDto> {
    const tradeBotService = await this.tradeBotServiceFactory.fromBotId(botId);
