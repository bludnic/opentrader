...
import {
  ...
  TradeBotOrderServiceFactorySymbol,
} from './trade-bot-orders-service.factory';
import {
...
export class TradeBotSyncService {
  ...
  async sync() {
    ...
    for (const bot of bots) {
      ...
      const tradeBotService = await this.tradeBotServiceFactory.fromBotId(
        bot.id,
      );
      ...
      if (anyOrderWasSynced) {
        ...
        const ordersService = await this.tradeBotOrdersServiceFactory.fromBotId(
          updatedBot.id,
        );
