...
export class SmartTradePrivateService {
  ...
  ): Promise<SyncedSmartTradeDto[]> {
    const smartTrades =
      await this.firestore.smartTrade.findAllByExchangeAccountId(
        exchangeAccountId,
      );

    this.logger.debug(
      `[SmartTradeService] Start syncing Smart Trades of the exchange with ID: ${exchangeAccountId}`,
    );

    const syncedSmartTrades = await this.syncSmartTradesStatuses(smartTrades);

    this.logger.debug(
      `[SmartTradeService] End syncing Smart Trades of the exchange with ID: ${exchangeAccountId}`,
    );

    this.logger.debug(
      `[SmartTradeService] Start processing Smart Trades of the exchange with ID: ${exchangeAccountId}`,
    );

    const placedOrders = await this.processIdleSmartTrades(smartTrades);

    this.logger.debug(
      `[SmartTradeService] End processing Smart Trades of the exchange with ID: ${exchangeAccountId}`,
    );
   ...
   * @param smartTrades
   */
  private async processIdleSmartTrades(
  ...
  private async processIdleSmartTrade(
    smartTrade: ISmartTrade,
  ): Promise<IPlaceLimitOrderResponse | void> {
    const { buyOrder, sellOrder, baseCurrency, quoteCurrency, quantity } =
      smartTrade;
    ...
    if (buyOrder.status === OrderStatusEnum.Idle) {
      ...
      this.logger.debug(
        `[SmartTradePrivateService] BUY order of the Smart Trade ID: ${smartTrade.id} was placed`,
      );
      ...
      this.logger.debug(
        `[SmartTradePrivateService] SELL order of the Smart Trade ID: ${smartTrade.id} was placed`,
      );
      console.log('order', order);
  ...
  private async syncSmartTradesStatuses(
    smartTrades: ISmartTrade[],
  ): Promise<SyncedSmartTradeDto[]> {
    ...
    for (const smartTrade of smartTrades) {
      this.logger.log(
        `[SmartTradeService] Start syncing SmartTrade: #${smartTrade.id}`,
      );
      ...
      this.logger.log(
        `[SmartTradeService] End syncing SmartTrade: #${smartTrade.id}`,
      );
    }
   ...
   * @param smartTrade
   */
  private async syncSmartTradeStatus(
    smartTrade: ISmartTrade,
  ): Promise<SyncedSmartTradeDto | void> {
    const { buyOrder, sellOrder, baseCurrency, quoteCurrency } = smartTrade;
