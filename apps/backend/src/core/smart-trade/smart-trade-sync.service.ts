...
} from './smart-trade-private-service.factory';
...
export class SmartTradeSyncService {
  ...
  async sync() {
    ...
    this.logger.debug(
      `[SmartTradeSyncService] Start syncing process: ${startSyncTime}`,
    );
    ...
    if (exchangeAccounts.length === 0) {
      this.logger.debug(
        `[SmartTradeSyncService] No exchange accounts. Skip sync process.`,
      );
    }

    this.logger.debug(
      `[SmartTradeSyncService] Exchange accounts: ${exchangeAccounts.length}`,
    );
    ...
    for (const exchangeAccount of exchangeAccounts) {
      ...
      const smartTradeService =
        this.smartTradePrivateServiceFactory.fromExchangeAccount(
          exchangeAccount,
        );
      const syncResponse = await smartTradeService.syncSmartTrades(
        exchangeAccount.id,
      );
    ...
    this.logger.debug(
      `[SmartTradeSyncService] End syncing process: ${endSyncTime}`,
    );
  }
}
