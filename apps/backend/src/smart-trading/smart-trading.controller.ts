import { SmartTradingEndpoint } from '@bifrost/swagger';
import {
  Body,
  Controller,
  Get,
...
import {
  ...
  SmartTradePrivateServiceFactorySymbol,
} from 'src/core/smart-trade/smart-trade-private-service.factory';
import {
  SmartTradePublicServiceFactory,
  SmartTradePublicServiceFactorySymbol,
} from 'src/core/smart-trade/smart-trade-public-service.factory';
...
@ApiTags(SmartTradingEndpoint.tagName())
export class SmartTradingController {
  ...
  @Get()
  @ApiOperation(SmartTradingEndpoint.operation('getSmartTrades'))
  async getSmartTrades(
    @FirebaseUser() user: IUser,
  ): Promise<GetSmartTradesListResponseDto> {
    const smartTradePublicService =
  ...
  @Get('/info/:id')
  @ApiOperation(SmartTradingEndpoint.operation('getSmartTrade'))
  async getSmartTrade(
    @Param('id') smartTradeId: string,
  ): Promise<GetSmartTradeResponseBodyDto> {
    const smartTradePublicService =
      this.smartTradePublicServiceFactory.create();
  ...
  @Post('/create')
  @ApiOperation(SmartTradingEndpoint.operation('createSmartTrade'))
  ...
  async syncSmartTrades(
    ...
    const smartTradePrivateService =
      await this.smartTradePrivateServiceFactory.fromExchangeAccountId(
        exchangeAccountId,
      );

    try {
      const syncedSmartTrades = await smartTradePrivateService.syncSmartTrades(
        exchangeAccountId,
      );
